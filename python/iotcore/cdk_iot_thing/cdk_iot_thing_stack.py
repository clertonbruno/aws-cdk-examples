...
class CdkIotThingStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        provider = CustomResourceProvider.Provider(
            self, "IoTCertProvider", on_event_handler=cert_lambda
        )

        certificate = CustomResource(
            self, "IoTCertCustomResource", service_token=provider.service_token
        )

        # Create a policy of the certificate
        cfn_policy = iot.CfnPolicy(
            self,
            "CfnPolicy",
            policy_document={
                "Version": "2012-10-17",
        ...
        iot_topic_rule_sql = (
            "SELECT color AS rgb FROM 'device/color' WHERE temperature > 50"
        )
