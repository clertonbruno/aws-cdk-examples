...
class ApiStagesLambdaStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        # Define the REST API
        api = apigw.RestApi(
            self, "DeploymentStagesAPI", rest_api_name="MultiStageApi", deploy=False
        )

        # Define GET method for /stage_info
        stage_info_resource = api.root.add_resource("stage_info")
        stage_info_get_method = stage_info_resource.add_method(
            "GET",
        ...
        prod_alias = lambda_.Alias(
            self, "ProdAlias", alias_name="prod", version=version
        )
        ...
        prod_stage = apigw.Stage(
            ...
            stage_name="prod",
            variables={"lambdaAlias": prod_alias.alias_name},
        )
        ...
        prod_alias.add_permission(
            "ProdStageInvokeProdAliasPermission",
        ...
        dev_alias = lambda_.Alias(self, "DevAlias", alias_name="dev", version=version)
        ...
        dev_stage = apigw.Stage(
            ...
            stage_name="dev",
            variables={"lambdaAlias": dev_alias.alias_name},
        )

        dev_alias.add_permission(
            "DevStageInvokeDevAliasPermission",
            principal=iam.ServicePrincipal("apigateway.amazonaws.com"),
            action="lambda:InvokeFunction",
            source_arn=api.arn_for_execute_api(
                method=stage_info_get_method.http_method,
        ...
        test_alias = lambda_.Alias(
            self, "TestAlias", alias_name="test", version=version
        )

        test_deployment = apigw.Deployment(self, "TestApiDeployment", api=api)
        test_stage = apigw.Stage(
            self,
            "TestApiStage",
            deployment=test_deployment,
            stage_name="test",
            variables={"lambdaAlias": test_alias.alias_name},
        )

        test_alias.add_permission(
            "TestStageInvokeTestAliasPermission",
            principal=iam.ServicePrincipal("apigateway.amazonaws.com"),
            action="lambda:InvokeFunction",
            source_arn=api.arn_for_execute_api(
                method=stage_info_get_method.http_method,
