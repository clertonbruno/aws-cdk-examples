from aws_cdk import Aws, CfnOutput, Stack, aws_lambda, aws_cloudwatch, aws_iam
...
class LambdaCloudwatchDashboardStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        lambda_function = awslambda_.Function(
            self,
            "lambda_function",
            runtime=awslambda_.Runtime.PYTHON_3_7,
            handler="lambda-handler.main",
            code=awslambda_.Code.from_asset("./lambda"),
        )
        ...
        invocations_widget = aws_cloudwatch.GraphWidget(
            title="Invocations", left=[lambda_function.metric_invocations()], width=24
        )
        ...
        duration_widget = aws_cloudwatch.GraphWidget(
            title="Duration", left=[lambda_function.metric_duration()], width=24
        )

        throttles_widget = aws_cloudwatch.GraphWidget(
            title="Throttles", left=[lambda_function.metric_throttles()], width=24
        )
