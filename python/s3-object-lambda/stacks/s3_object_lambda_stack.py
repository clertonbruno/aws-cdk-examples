...
class S3ObjectLambdaStack(Stack):
    def __init__(self, app: App, id: str) -> None:
        ...
        retrieve_transformed_object_lambda = lambda_.Function(
            self,
            "retrieve_transformed_obj_lambda",
            runtime=lambda_.Runtime.PYTHON_3_8,
            handler="index.handler",
            code=lambda_.Code.from_asset("lambda/retrieve_transformed_object_lambda"),
        )

        # Object lambda s3 access
        retrieve_transformed_objectlambda_.add_to_role_policy(
            iam.PolicyStatement(
        ...
        retrieve_transformed_objectlambda_.add_permission(
        ...
        if retrieve_transformed_objectlambda_.role is not None:
            policy_doc = iam.PolicyDocument()
            policy_statement = iam.PolicyStatement(
                effect=iam.Effect.ALLOW,
                actions=["s3:GetObject"],
                principals=[
                    iam.ArnPrincipal(retrieve_transformed_objectlambda_.role.role_arn)
                ],
        ...
        object_lambda_ap = s3_objectlambda_.CfnAccessPoint(
            self,
            "s3_object_lambda_ap",
            name=OBJECT_LAMBDA_ACCESS_POINT_NAME,
            object_lambda_configuration=s3_objectlambda_.CfnAccessPoint.ObjectLambdaConfigurationProperty(
                supporting_access_point=self.access_point,
                transformation_configurations=[
                    s3_objectlambda_.CfnAccessPoint.TransformationConfigurationProperty(
                        actions=["GetObject"],
                        content_transformation={
                            "AwsLambda": {
                                "FunctionArn": f"{retrieve_transformed_objectlambda_.function_arn}"
                            }
                        },
                    )
                ],
        ...
        CfnOutput(
            self,
            "objectLambdaArn",
            value=retrieve_transformed_objectlambda_.function_arn,
        )
